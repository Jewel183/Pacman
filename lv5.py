...
class Level5:
    def __init__(self):
        ...
        pygame.display.set_caption("Level 5: Four Ghost")
    ...
    def run(self):
        ...
        found_pacman = [False, False, False, False]  
        ...
        while self.running:
            ...
            # DFS
            if not found_pacman[1]:
                path2, expanded_nodes2, memory_usage2, search_time2 = dfs(tuple(self.pink_ghost.grid_pos), tuple(self.pacman.grid_pos), self.maze, track_stats=True)
                
                self.pink_ghost.expanded_nodes = expanded_nodes2
                self.pink_ghost.memory_usage = memory_usage2
                self.pink_ghost.search_time = search_time2
                
                if path2:
                    self.pink_ghost.move_toward(path2, self.maze)
                
                if tuple(self.pink_ghost.grid_pos) == tuple(self.pacman.grid_pos):
                    found_pacman = True
                    time_to_catch = time.time() - self.start_time
                    print(f"Blue Ghost (BFS) bắt được Pac-Man sau {time_to_catch:.4f} giây!")
                    print(f"Thời gian tìm kiếm BFS: {self.pink_ghost.search_time:.4f} giây")
                    print(f"Số nút mở rộng: {self.pink_ghost.expanded_nodes}")
                    print(f"Bộ nhớ sử dụng: {self.pink_ghost.memory_usage} bytes")
